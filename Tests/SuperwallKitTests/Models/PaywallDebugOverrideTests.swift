...
struct PaywallDebugOverrideTests {
  ...
  @Test func presentationInfo_fromIdentifier_carriesAllOverrides() {
    let info = PresentationInfo.fromIdentifier(
      "abc",
      freeTrialOverride: true,
      attributeOverrides: ["plan": "pro"]
    )

    #expect(info.attributeOverrides == ["plan": "pro"])
    #expect(info.freeTrialOverride == true)
    #expect(info.identifier == "abc")
  }
}
